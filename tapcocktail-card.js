...
class TapCocktailLibraryCard extends HTMLElement {
  ...
  _render() {
    ...
    const hiddenCocktailCount = cocktailTab ? Math.max(0, allItems.length - 3) : 0;
    const items = cocktailTab && !this._showAllCocktails ? allItems.slice(0, 3) : allItems;
    ...
    else content = `<div class="grid">${items.map((item) => cocktailTab ? this._renderCocktail(item) : this._renderIngredient(item)).join("")}</div>${cocktailTab && allItems.length > 3 ? `<button class="secondary-button list-toggle">${this._showAllCocktails ? "Vis færre" : `Vis resten (${hiddenCocktailCount})`}</button>` : ""}`;
    let dialog = "";
  ...
  _bind() {
    ...
    root.querySelectorAll("[data-tab]").forEach((button) => button.addEventListener("click", () => { this._tab = button.dataset.tab; this._search = ""; this._category = "all"; this._showAllCocktails = false; this._render(); }));
    root.querySelector(".search")?.addEventListener("input", (event) => { this._search = event.target.value; this._showAllCocktails = false; this._render(); root.querySelector(".search")?.focus(); });
    ...
    root.querySelector(".list-toggle")?.addEventListener("click", () => { this._showAllCocktails = !this._showAllCocktails; if (!this._showAllCocktails) this._expandedCocktailId = null; this._render(); });
